...
var CanvasExploder = class {
  ...
  async explodeFileNode(canvas, originalNode) {
    ...
    const newEdgesData = [];
    let createdCount = 0;
    const originalData = (_b = canvas.getData().nodes.find((n) => n.id === originalNode.id)) != null ? _b : {};
  ...
  async explodeTextNode(canvas, originalNode) {
    var _a, _b, _c;
    const rawText = (_a = originalNode.text) != null ? _a : "";
    ...
    const originalWidth = (_b = originalNode.width) != null ? _b : 0;
    ...
    const originalData = (_c = canvas.getData().nodes.find((n) => n.id === originalNodeId)) != null ? _c : {};
    let currentY = baseY;
    const nodeStack = [];
    const newNodesData = [];
    const newEdgesData = [];
...
var EnhancedCanvas = class extends import_obsidian6.Plugin {
  ...
  async processEdgeUpdate(e) {
    ...
    if (!fromNode || !toNode)
      return;
    const fromFilePath = (_d = fromNode.filePath) != null ? _d : (_c = fromNode.file) == null ? void 0 : _c.path;
    const toFilePath = (_f = toNode.filePath) != null ? _f : (_e = toNode.file) == null ? void 0 : _e.path;
  ...
  registerCanvasAutoLink() {
    const processNodeUpdate = async (e) => {
      ...
      const sameFileNodeIds = new Set(
        nodes.filter((node) => node.file === fromNode.filePath).map((node) => node.id)
      );
    ...
    const updateTargetNode = (0, import_obsidian6.debounce)((e) => {
    ...
    const updateOriginalNode = async (edge) => {
      ...
      const sameFileNodes = nodes.filter((node) => node.file === fromFilePath);
    ...
    const patchCanvas = () => {
      ...
      if (canvasPatched)
        return false;
      const canvasView = (_a = this.app.workspace.getLeavesOfType("canvas")[0]) == null ? void 0 : _a.view;
